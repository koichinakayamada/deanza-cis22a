...
// Function to find the maximum length of course titles for formatting
...
// Displays all courses stored in the vectors with aligned units
void displayCourses(const vector<int> &units, const vector<string> &titles)
{
    ...
    {
        cout << left << setw(maxTitleLength + 4) << titles[i] << right << setw(5) << units[i] << endl;
    }
}

// Displays courses with at least a specified number of units with aligned units
void displayCoursesWithMinUnits(const vector<int> &units, const vector<string> &titles, int minUnits)
{
    int maxTitleLength = findMaxTitleLength(titles);
...
int main()
{
    ...
    } while (choice != 'E' && choice != 'e'); // Loop until 'E' or 'e' is entered for exit
